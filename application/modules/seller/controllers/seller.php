...
class Seller extends MX_Controller {
    ...
    function __construct() {
        ...
        if ($login_email == NULL || $reg_id == NULL || $type == NULL || $type == "" || $type == 1 || $type == 3 || $type == 4 || $login_email == "") {
    ...
    public function index() {
        ...
        $reg_id = $this->session->userdata('reg_id');
        $data['get_user_data'] = $this->seller_model->get_user_data($reg_id);
        $data['all_product_info'] = $this->seller_model->get_all_product($reg_id);
        ...
        if ($count_num <= 0) {
            ...
            $data['category'] = $this->seller_model->get_all_category();
...
//        print_r($data);die();
        $this->seller_model->update_profile_info($data,$reg_id);
    ...
    public function all_market_product() {
        ...
        $reg_id = $this->session->userdata('reg_id');
        $data['get_user_data'] = $this->seller_model->get_user_data($reg_id);
        $data['all_product_info'] = $this->seller_model->get_all_market_product($reg_id);
        $this->load->view('template/seller/headerlink');
        $this->load->view('template/seller/head', $data);
        //$this->load->view('template/seller/leftnav');
    ...
    public function edit_product() {
        ...
        $check_id = $this->seller_model->get_image_table_data($p_id);
        if ($check_id) {
            $data['p_details'] = $this->seller_model->get_product_details($p_id, $reg_id);
        } else {
            $data['p_details'] = $this->seller_model->get_product_details_w_image($p_id, $reg_id);
        }
...
//         die();
        ...
        $data['sub_category_name'] = $this->seller_model->get_sub_cat_name($data['p_details'][0]['sub_cat_id']);
...
//        echo $images_table_id;die();
        $data['image_details'] = $this->seller_model->image_details($images_table_id);
//        print_r($data);exit();
        $this->load->view('seller/view_main_image_modal',$data);
    }
    
    public function sub_image_details_modal() {
        $images_id = $this->input->post('images_id');
//        echo $images_id;die();
        $data['image_details'] = $this->seller_model->sub_image_details($images_id);
//        print_r($data);exit();
        $this->load->view('seller/view_sub_image_modal',$data);
    }
...
//            print_r($product_image);exit();
        $this->seller_model->update_main_image($product_id,$product_image);
        redirect('seller/edit_product/'.$product_id);
    ...
    public function update_sub_image() {
        ...
        $files = $_FILES;
            ...
            $product_image = $_FILES['userfile']['name'];
//            print_r($product_image);exit();
        $this->seller_model->update_sub_image($images_id,$product_image);
        redirect('seller/edit_product/'.$images_table_id);
    }
    
    public function delete_sub_images($images_id,$product_id) {
        $this->seller_model->delete_sub_image($images_id);
        redirect('seller/edit_product/'.$product_id);
    }
    ...
    public function edit_market_product() {
        ...
        $data['get_user_data'] = $this->seller_model->get_user_data($reg_id);
        $p_id = $this->uri->segment(3);
        $check_id = $this->seller_model->get_image_table_data_market($p_id);
        if ($check_id) {
            $data['p_details'] = $this->seller_model->get_market_product_details($p_id, $reg_id);
        } else {
            $data['p_details'] = $this->seller_model->get_market_product_details_w_image($p_id, $reg_id);
        }
    ...
    public function select_sub_category() {
        ...
        $sub_category = $this->seller_model->get_sub_category($cat_id);
    ...
    public function update_product_details($p_id) {
        ...
        $files = $_FILES;
         ...
         if(array_sum($_FILES['userfile']['error']) > 0) { 
           ...
           $this->seller_model->update_new_product($p_id, $data);
         } 
         else{
             $data['product_name'] = $this->input->post('product_name');
            $data['product_price'] = $this->input->post('product_price');
...
//            exit();
            ...
            $this->seller_model->insert_product_images($data1);
        }
}
        redirect('seller/all_product', 'refresh');
    }
    ...
    public function update_market_product_details($p_id) {
        ...
        if ($counter == 0) {
            ...
            $this->seller_model->update_new_market_product($p_id, $data1);
        } else {
            //upload image
            $this->seller_model->delete_market_product_sub_images($p_id);
            $this->load->library('upload');
            $files = $_FILES;
            $cpt = count($_FILES['userfile']['name']);
            for ($i = 0; $i < $cpt; $i++) {
                ...
                if ($i == 0) {
                    ...
                    $this->seller_model->insert_market_place_images($data2);
                }
            }
        }

        redirect('seller/all_market_product', 'refresh');
    }
    ...
    public function delete_product($p_id) {
        ...
        $this->load->helper('text');
        $data['about_info'] = $this->db->get('about')->row();
        $this->seller_model->delete_product_confirm($p_id);
        redirect('seller/all_product', 'refresh');
    }
    ...
    public function delete_market_product($p_id) {
        $this->load->helper('text');
        $data['about_info'] = $this->db->get('about')->row();
        $this->seller_model->delete_market_product_confirm($p_id);
        redirect('seller/all_market_product', 'refresh');
    }
    ...
    public function insert_product() {
        ...
        for ($i = 0; $i < $cpt; $i++) {
            ...
            if ($i == 0) {
                ...
                $last_product_id = $this->seller_model->insert_new_product($data1);
            } else {
                $data2 = array
                    (
                ...
                $this->seller_model->insert_product_images($data2);
            }
        }
        redirect('seller/all_product', 'refresh');
    }
    ...
    public function insert_product_marketplace() {
        ...
        for ($i = 0; $i < $cpt; $i++) {
            ...
            if ($i == 0) {
                ...
                $last_product_id = $this->seller_model->insert_new_market_product($data1);
            } else {
                $data2 = array
                    (
                    'images_table_id' => $last_product_id,
    ...
    public function change_password() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->seller_model->password_change($data, $reg_id);
        }
